...
{
    public partial class app_db_make_main : Form
   
    {
        ...
        void AddToAppstore()
        {
            ...
            string createStatement = @"create table if not exists onestore_app(" +
                "[id] GUID PRIMARY KEY, " +
                "[name_cn] NVARCHAR, " +
                "[name_en] NVARCHAR, " +
                "[name_other] NVARCHAR, " +
            ...
            { 
                ...
                string insertStatement = string.Format(@"insert into onestore_app({0}) values({1})", fieldstr, paramstr);
